...
事实：本轮继续把 opencode 作为 major_update 项目，重点观察多模型 CLI、计划、只读、审查等 coding agent 护栏。
推断：它的长期产品启发是“把安全流程做成默认体验”，而不是依赖用户自己写复杂提示词来约束 Agent。

## 2026-05-02 长期档案更新

事实：opencode release 页面继续出现 provider、workspace、TUI、插件和命令体验相关更新。
推断：开源 coding agent 的竞争正在从“能接多少模型”走向“能否适配真实团队工作区，并把复杂度藏在默认流程里”。
